...
NVS_TEST_INDEX = [8,13,16,21,26,31,34,56]
...
class NeRFDataset:
    def __init__(self, path, type='train', mode='dtu', preload=True, downscale=1, bound=0.33, n_test=10, camera_traj='else', scale=1.0):
        ...
        if self.type == 'test':
            ...
            if type == 'train':
                image_paths = [image_paths[ind] for ind in range(len(image_paths)) if ind not in NVS_TEST_INDEX]
                mask_paths = [mask_paths[ind] for ind in range(len(mask_paths)) if ind not in NVS_TEST_INDEX]
                self.poses = self.poses[[ind for ind in range(len(self.poses)) if ind not in NVS_TEST_INDEX]]
                self.intrinsics = self.intrinsics[[ind for ind in range(len(self.intrinsics)) if ind not in NVS_TEST_INDEX]]
            elif type == 'val':
                image_paths = [image_paths[ind] for ind in range(len(image_paths)) if ind in NVS_TEST_INDEX]
                mask_paths = [mask_paths[ind] for ind in range(len(mask_paths)) if ind in NVS_TEST_INDEX]
                self.poses = self.poses[[ind for ind in range(len(self.poses)) if ind in NVS_TEST_INDEX]]
                self.intrinsics = self.intrinsics[[ind for ind in range(len(self.intrinsics)) if ind in NVS_TEST_INDEX]]
            elif type == 'mesh':
